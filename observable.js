(() => {
  ...
  function closeASubscription(subscriber) {
    ...
    // 3. Signal abort subscriber’s subscription controller.
  ...
  function subscribeTo(observable, observer, options = {}) {
    ...
    if (subscribe) subscribe(subscriber)
  ...
  class Subscriber {
    ...
    addTeardown(teardown) {
      ...
      if (privateState.has(this)) privateState.get(this).teardowns.push(teardown)
  ...
  class Observable {
    ...
    flatMap(mapper) {
      ...
      return new Observable(subscriber => {
        ...
        function switchmapProcessNextValueSteps(value) {
          ...
          subscriber.signal.addEventListener('abort', () => dependantAbortController.abort(), { once: true })
          if (activeInnerAbortController)
            activeInnerAbortController.signal.addEventListener('abort', () => dependantAbortController.abort(), { once: true })
    ...
    forEach(callback, options = {}) {
      ...
      let dependantAbortController = new AbortController()
      visitorCallbackController.signal.addEventListener('abort', e => dependantAbortController.abort(e.reason))
      if (options.signal) options.signal.addEventListener('abort', e => dependantAbortController.abort(e.reason))
    ...
    every(predicate, options = {}) {
      ...
      // current realm.
      let dependantAbortController = new AbortController()
      controller.signal.addEventListener('abort', e => dependantAbortController.abort(e.reason))
      if (options.signal) options.signal.addEventListener('abort', e => dependantAbortController.abort(e.reason))
      let internalOptions = { signal: dependantAbortController.signal }
      // 4. If internal options’s signal is aborted, then:
      if (internalOptions.signal.aborted) {
